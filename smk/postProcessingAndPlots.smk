...
from RDPMSpecIdentifier.plots import DEFAULT_TEMPLATE, COLOR_SCHEMES
    ...
    run:
        ...
        if isinstance(ids, list):
            ...
            for key, value in ids.items():
                df = df[df[key] == value]
            tcol = "old_locus_tag"
            rdpmsids = df["RDPMSpecID"]
        rows = math.ceil(len(rdpmsids)/2)
        fig = plot_protein_distributions(rdpmsids, rdpmsdata=data, colors=COLOR_SCHEMES["Dolphin"], title_col=tcol, cols=2, rows=rows, vertical_spacing=0.1 * (1/rows), horizontal_spacing=0.075)
...
rule plotRuntime:
    ...
    output:
        html = "Pipeline/plots/benchmark.html",
        svg = "Pipeline/plots/benchmark.svg"
    run:
        import plotly.graph_objs as go
        ...
        for col in range(1, 3):
            ...
            fig.add_trace(
                go.Bar(
                    x=df[(df["ANOSIM"] == False) & (df["Tool"] == "RDPMSpecIdentifier")]["Replicates per group"],
                    y=df[(df["ANOSIM"] == False) & (df["Tool"] == "RDPMSpecIdentifier")][name],
                    error_y=dict(
                        type='data',# value of error bar given in data coordinates
                        array=df[(df["ANOSIM"] == False) & (df["Tool"] == "RDPMSpecIdentifier")][f"stdev_{name}"],
                        visible=True,
                        color="black"
                    ),

                    legendgroup="RDPMSpecIdentifier",
                    legendgrouptitle=dict(text="RDPMSpecIdentifier"),
            ...
            fig.add_trace(
                go.Bar(
                    x=df[(df["ANOSIM"] == True) & (df["Tool"] == "RDPMSpecIdentifier")]["Replicates per group"],
                    y=df[(df["ANOSIM"] == True) & (df["Tool"] == "RDPMSpecIdentifier")][name],
                    error_y=dict(
                        type='data',# value of error bar given in data coordinates
                        array=df[(df["ANOSIM"] == True) & (df["Tool"] == "RDPMSpecIdentifier")][f"stdev_{name}"],
                        visible=True,
                        color="black"
                    ),
                    legendgroup="RDPMSpecIdentifier",
        ...
        fig.update_layout(template=DEFAULT_TEMPLATE, width=800, height=400)
...
rule createBubblePlot:
    ...
    run:
        ...
        import pandas as pd
        import math
        from RDPMSpecIdentifier.plots import plot_dimension_reduction
        with open(input.file) as handle:
            data = RDPMSpecData.from_json(handle.read())
        data.calc_distribution_features()
        if wildcards.highlight == "top10":
            ids = data.df[data.df["Rank"] <= 10]["RDPMSpecID"]
        else:
        ...
        fig.layout.update(template=DEFAULT_TEMPLATE, width=800, height=600, legend=dict(visible=False), margin=dict(t=10))
        fig.write_image(output.svg)
        fig.write_html(output.html)